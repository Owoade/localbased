...
export default abstract class ServerController{
    static async create( req: Request, res: Response){
        ...
        const NO_COLLECTION_DIR = !fs.existsSync(`${CWD}/db/collections/${collectionName}`);
    ...
    static async getAll(req: Request, res: Response){
        ...
        if( documents.length === 0 ) return res.send(`Collection '${collectionName}' doesn't exist`)
  
        documents.forEach( (doc)=>{
          const data= JSON.parse(fs.readFileSync(`db/${doc}`,'utf-8'));
          all_docs.push(data)
        })
    ...
    static async getOne (req: Request, res: Response){
        ...
        try{
          const doc_json = fs.readFileSync(`${CWD}/db/${collectionName}/${id}.json`);
    ...
    static async update(req: Request, res: Response){
        ...
        try{
          ...
          const doc_json = fs.readFileSync(filePath);
          ...
          fs.writeFileSync(filePath, JSON.stringify(new_doc));
    ...
    static async delete(req: Request, res: Response){
        ...
        fs.unlinkSync(`${CWD}/db/collections/${collectionName}/${id}.json`)
